...
public class MainWindow extends JFrame {
    ...
    // Новая таблица для настроек (аналог tuneTableDT из C#)
    private JTable tuneTable;
    private DefaultTableModel tuneTableModel;
    ...
    private void initializeComponents() {
        ...
        // Новая таблица для настроек (tuneTable)
        String[] tuneColumns = {"Настройка"};
        tuneTableModel = new DefaultTableModel(tuneColumns, 0);
        tuneTable = new JTable(tuneTableModel);
        tuneTable.setRowHeight(25);
        ...
        // Обновленный ComboBox с примерами из C# версии
        exampleComboBox = new JComboBox<>(new String[]{
                "Прямая адресация",
    ...
    private void setupLayout() {
        ...
        // Группа 2 - обновленная с таблицей настроек
        groupBox2.setBounds(347, 12, 444, 643);
        label3.setBounds(140, 12, 184, 17);

        // Таблица символов (уменьшена для места под tuneTable)
        JScrollPane symbolScroll = new JScrollPane(symbolTable);
        symbolScroll.setBounds(6, 32, 430, 200);
        groupBox2.add(symbolScroll);

        // Новая таблица настроек
        JLabel labelTune = new JLabel("Таблица настроек");
        labelTune.setFont(new Font("Microsoft Sans Serif", Font.PLAIN, 10));
        labelTune.setBounds(140, 240, 184, 17);
        groupBox2.add(labelTune);

        JScrollPane tuneScroll = new JScrollPane(tuneTable);
        tuneScroll.setBounds(6, 260, 430, 180);
        groupBox2.add(tuneScroll);

        label5.setBounds(194, 450, 61, 17);
        firstPassErrorTextBox.setBounds(6, 470, 430, 127);

        JScrollPane errorScroll = new JScrollPane(firstPassErrorTextBox);
        errorScroll.setBounds(6, 470, 430, 127);
    ...
    private void setupEventHandlers() {
        ...
        // Обработчик редактирования таблицы операционных кодов (из C#)
        operationCodeTable.getModel().addTableModelListener(e -> {
            clear();
        });
    }
        ...
        switch (param) {
            case 0: // Один шаг
                if (currentRow + 1 <= sourceCodeArray.length) {
                    // Обновленный вызов DoPass с tuneTable
                    if (!core.doPass(sourceCodeArray, operationCodeArray, symbolTable, binaryCodeTable, currentRow, tuneTable)) {
            ...
            case 2: // Запуск/Продолжить
                for (int i = currentRow; i <= sourceCodeArray.length; i++) {
                    if (i + 1 <= sourceCodeArray.length) {
                        // Обновленный вызов DoPass с tuneTable
                        if (!core.doPass(sourceCodeArray, operationCodeArray, symbolTable, binaryCodeTable, i, tuneTable)) {
                            writeError(firstPassErrorTextBox, core.getErrorText());
                            stop();
                            return;
                        }
    ...
    private void clear() {
        ...
        tuneTableModel.setRowCount(0); // Очистка таблицы настроек
    }
    ...
    private boolean parser() {
        ...
        for (int i = 0; i < str.length; i++) {
            ...
            if (temp.length >= 3) {
                if (temp[2].indexOf('"') == 0 && temp[temp.length - 1].lastIndexOf('"') == temp[temp.length - 1].length() - 1) {
                    for (int j = 3; j < temp.length; j++) {
                        temp[2] += " " + temp[j];
                        temp[j] = "";
                    }
                } else if (temp[1].indexOf('"') == 0 && temp[temp.length - 1].lastIndexOf('"') == temp[temp.length - 1].length() - 1) {
            ...
            if (temp.length <= 4) {
                if (temp.length == 1) {
                    ...
                    if (dC.checkDirective(temp[0]) || core.findCode(temp[0], operationCodeArray) != -1) {
                        // Проверка типа адресации (улучшенная из C#)
                    ...
                    if (dC.checkDirective(temp[0]) || core.findCode(temp[0], operationCodeArray) != -1) {
                        // Обработка строк с кавычками для 3 элементов
                        if (temp[2].indexOf('"') == 0) {
                            sourceCodeArray[i][1] = temp[0];
                    ...
                    if (dC.checkDirective(temp[1]) || core.findCode(temp[1], operationCodeArray) != -1) {
                        ...
                        // Обработка строк с кавычками для 4 элементов
                        if (temp[2].indexOf('"') == 0 && temp[3].lastIndexOf('"') == temp[3].length() - 1) {
                            sourceCodeArray[i][0] = temp[0];
                            sourceCodeArray[i][1] = temp[1];
    ...
    private void exampleComboBoxSelectedIndexChanged() {
        clear();
        int selectedIndex = exampleComboBox.getSelectedIndex();
        if (selectedIndex == 0) {
