...
public class DataCheck {
    ...
    public boolean checkEmptyAddress(List<List<String>> symbolTB, int[] num) {
        num[0] = -1;
        if (!symbolTB.get(0).isEmpty()) {
            for (int i = 0; i < symbolTB.get(0).size(); i++) {
                if (!symbolTB.get(2).get(i).isEmpty()) {
                    num[0] = i;
                    return false;
                }
            }
